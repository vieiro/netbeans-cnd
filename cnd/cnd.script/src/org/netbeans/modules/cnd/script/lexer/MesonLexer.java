...
import org.netbeans.modules.cnd.api.script.MesonTokenId;
 ...
 *
 */
public class MesonLexer implements Lexer<MesonTokenId>  {
    private static final Set<String> functions = new HashSet<String>();
    private static final Set<String> objects = new HashSet<String>();
    ...
    private final LexerRestartInfo<MesonTokenId> info;

    MesonLexer(LexerRestartInfo<MesonTokenId> info) {
        this.info = info;
    }

    @Override
    public Token<MesonTokenId> nextToken () {
        ...
        switch (i) {
            ...
            case '.':
            case '%':
                return info.tokenFactory().createToken(MesonTokenId.OPERATOR);
            case ' ':
            case '\n':
            case '\r':
            case '\t':
                ...
                return info.tokenFactory().createToken(MesonTokenId.WHITESPACE);
            case '#':
                do {
                    i = input.read();
                } while ((i != '\n') && (i != '\r') && (i != LexerInput.EOF));
                return info.tokenFactory().createToken(MesonTokenId.COMMENT);
            case '\'':
                ...
                if (input.read() == '\'') {
                    if (input.read() == '\'') {
                        ...
                        return info.tokenFactory().createToken(MesonTokenId.STRING);
                    } else {
                        input.backup(2);
                    }
                } else {
                    ...
                    if (i == 'x') {
                        do {
                            i = java.lang.Character.toLowerCase(input.read());
                                 ...
                                 || ((i >= 'a') && (i <= 'f')));
                        input.backup(1);
                        return info.tokenFactory().createToken(MesonTokenId.NUMBER);
                    } else if (i == 'o') {
                        do {
                            i = input.read();
                        } while ((i >= '0') && (i <= '7'));
            ...
            default:
                if (   (i >= 'a' && i <= 'z')
                    ...
                    if (keywords.contains(token)) {
                        return info.tokenFactory().createToken(MesonTokenId.KEYWORD);
                    } else if (functions.contains(token)) {
                        return info.tokenFactory().createToken(MesonTokenId.FUNCTION);
                    } else if (objects.contains(token)) {
                        return info.tokenFactory().createToken(MesonTokenId.OBJECT);
                    }
                    return info.tokenFactory().createToken(MesonTokenId.IDENTIFIER);
                }
                return info.tokenFactory().createToken(MesonTokenId.ERROR);
        }
    }
